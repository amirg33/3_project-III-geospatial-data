...
def midpoint_coordinates_sf():
    ...
    sflat, sflon = get_city_midpoint(df, "San Francisco")
    
    # Check if the midpoint is None (no data)
    if sflat is None or sflon is None:
        print("No midpoint data available for San Francisco.")
        return None, None
...
def midpoint_coordinates_ny():
    ...
    nylat, nylon = get_city_midpoint(df, "New York")
    
    # Return the computed midpoint coordinates
    return nylat, nylon

def midpoint_coordinates_ldn():
    """
    ...
    ldnlat, ldnlon = get_city_midpoint(df, "London")
    
    # Check if the midpoint is None (no data)
    if ldnlat is None or ldnlon is None:
        print("No midpoint data available for London.")
        return None, None

    # Return the computed midpoint coordinates
    return ldnlat, ldnlon
